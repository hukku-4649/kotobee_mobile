...
class VocabGameController extends Controller
{
    ...
    {        
        ...
        if ($index >= count($questions)) {
            ...
            // クリア後にリロードされた（＝ finish 済み）なら再スタート
            // if (session('vocab_finished_sent') && session('vocab_finished_stage_id')) {
            //     $stageId = session('vocab_finished_stage_id');
            //     return redirect()->route('vocab.mobile.start', ['stage_id' => $stageId]);
            // }
            ...
            if (session('vocab_finished_sent') && session('vocab_finished_stage_id')) {
                ...
                // ★ ここで消す（これが重要：一回だけにする）
                session()->forget(['vocab_finished_sent', 'vocab_finished_stage_id']);
            
                return redirect()->route('vocab.mobile.start', ['stage_id' => $stageId]);
            }
            

            // まだ finish を呼んでない（初回クリア直後）なら finish用ビューを返す
            return view('game.vocab.mobile.kana', [
                'question' => null,
                'chars' => [],
                'shuffled' => [],
                'isLast' => true,
        ...
        if ($current['type'] === 'choice') {
            ...
            return view('game.vocab.mobile.choice', [
                'question' => $word,
                'choices' => $choices,
                'isLast' => $isLast, // ★ 追加
            ]);
    ...
    public function checkChoice(Request $request)
    {
        ...
        if ($request->answer !== $word->word) {
            return redirect()->route('vocab.mobile.show')
                ->with('error', true);
        }
    ...
    public function checkKana(Request $request)
    {
        ...
        if ($answer === $correct) {
            ...
            return redirect()->route('vocab.mobile.show')->with('correct', true);
        }

        session()->forget('last_answer');
        session(['isLast' => false]);

        return redirect()->route('vocab.mobile.show')->with('error', true);
    }
    ...
    public function finish()
    {
        ...
        // 二重保存防止（超重要）
        if (session('vocab_finished_sent')) {
        ...
        // finish を1回実行した印
        session([
            'vocab_finished_sent' => true,
            'vocab_finished_stage_id' => $stageId,
        ]);
