...
function send_result(score) {
    ...
    fetch('/game/grammar/mobile/save', {
        method: 'POST',
        headers: {
...
function destroyGrammarGame() {
    try {
      if (window.grammarGame) {
        // 入力イベントを止めたい場合（任意）
        // window.grammarGame.input?.enabled = false;
  
        window.grammarGame.destroy(true); // canvas / events / timers も破棄
        window.grammarGame = null;
      }
    } catch (e) {
      console.warn("grammarGame destroy failed:", e);
      window.grammarGame = null;
    }
  
    // ルートDOMを掃除（残骸対策。任意だけど効果あり）
...
class MainScene extends Phaser.Scene {
    ...
    create() {   
        ...
        this.conveyor_max_speed = 6000;  // フェーズアウト中の最高速度（px/s）
        this.conveyor_accel     = 600;  // 加速量（px/s^2）
        this.conveyor_decel     = 100;  // 減速量（px/s^2）
        this.conveyor_phasein_start_speed = 800; // フェーズイン時の開始スピード        
