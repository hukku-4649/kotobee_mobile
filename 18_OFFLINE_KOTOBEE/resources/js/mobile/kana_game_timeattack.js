...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function postToFlutter(obj) {
        try {
            if (window.FlutterPostMessage && typeof window.FlutterPostMessage.postMessage === "function") {
                window.FlutterPostMessage.postMessage(JSON.stringify(obj));
            } else {
                console.warn("FlutterPostMessage channel not found");
            }
        } catch (e) {
            console.warn("postToFlutter error:", e);
        }
    }
    ...
    async function sendResult(timeSec) {
        ...
        try {
            const res = await fetch("/game/kana/mobile/save", {
                method: "POST",
                headers: {
            ...
            postToFlutter({
                ...
                payload: {
                    mode: "timeattack",
                    score: isNaN(timeNum) ? 0 : timeNum, // 今回の記録(秒)を入れる
                    rank: data?.my_rank ?? null,
                    best_time: (data?.my_best ?? null), // 秒
